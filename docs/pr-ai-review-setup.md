...
本仓库的 `pr-ai-review.yml` 使用 `pull_request_target`：它在 PR 创建、更新或重新打开时，使用目标分支中可信的工作流和审查规则读取 diff，调用 DeepSeek `deepseek-v4-pro`，并更新一条固定格式的 Code Review 报告。
...
首次包含本工作流的 PR 不会自动生成 AI 审查报告：`pull_request_target` 只使用 `main` 分支中已经存在的工作流定义。因此应人工检查该 PR，确认它只检出 `${{ github.event.pull_request.base.sha }}`、没有执行 PR 代码或打印环境变量后，使用 Squash and merge 合并到 `main`。

## 4. 验证自动审查

1. 从更新后的 `main` 创建一个内部测试分支，做一个无敏感信息的小文档修改。
2. 创建以 `main` 为 base 的 PR。
...
确认测试 PR 已成功跑出一次 `pr-security-gate` 后，进入 `Settings → Branches → Branch protection rules`，为 `main` 新建或编辑规则：
